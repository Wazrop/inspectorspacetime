(function (thisObj) {
  ...
  var thisComp,
    easeLib = {};
  var exp_counter =
    'var sTime = marker.key("Start").time; var eTime = marker.key("End").time; var countTime = Math.max(time - sTime, 0); countTime = Math.min(countTime, eTime - sTime); var counter = Math.round(countTime * 1000); var playIcon = (time > sTime && time < eTime) ? "\u25ba " : "\u25a0 "; playIcon + counter + "ms";';
    ...
    try {
      ...
      for (var _i = 0, selKeys_2 = selKeys; _i < selKeys_2.length; _i++) {
        ...
        for (var i = 0; i < keys.length - 1; i++) {
          // Loop through all keyframes, not just pairs
  ...
  function getPropSpec(actKey) {
    ...
    if (
      !layer.threeDLayer &&
      ((_a = valChange === null || valChange === void 0 ? void 0 : valChange.start) === null || _a === void 0 ? void 0 : _a.length) > 2
    ) {
      valChange.start.pop();
      valChange.end.pop();
    }
    ...
    var x1 = 5,
      ...
      y2 = 5;
    if (
      prop.keyOutInterpolationType(actKey.keys[0]) == KeyframeInterpolationType.LINEAR &&
    ...
    return {
      ...
      duration: duration,
      ease: [x1, y1, x2, y2],
      delay: prop.keyTime(keys[0]) - thisComp.displayStartTime,
    };
  }
  ...
  function buildUI() {
    ...
    myPanel.spacing = 4;
    myPanel.margins = 12;
    var btn_getSpec = myPanel.add("button", undefined, undefined, {
      name: "btn_getSpec",
    });
    btn_getSpec.text = "Get specs from selected keys";
    btn_getSpec.alignment = ["fill", "top"];
    var tpanel1 = myPanel.add("tabbedpanel", undefined, undefined, {
      name: "tpanel1",
    });
    ...
    var btn_saveJSON = tab3.add("button", undefined, undefined, {
      name: "btn_saveJSON",
    });
    ...
    var btn_newCounter = group1.add("button", undefined, undefined, {
      name: "btn_newCounter",
    });
    btn_newCounter.helpTip = "Create a time counter layer";
    btn_newCounter.text = "New counter";
    btn_newCounter.justify = "left";
    var btn_settings = group1.add("button", undefined, undefined, {
      name: "btn_settings",
    });
    ...
    var btn_help = group1.add("button", undefined, undefined, {
      name: "btn_help",
    });
    btn_help.helpTip = "Guide";
    btn_help.text = "Learn stuff";
    btn_help.justify = "right";
    group1.add("staticText", undefined, "v" + scriptVersion, {
      name: "btn_help",
    });
